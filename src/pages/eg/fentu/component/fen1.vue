...
<template>
  <div>
    <div>
      <!-----------------搜索框------------->
      <div id="searchId" style="text-align: left">
        <label for="search" style="line-height:32px;height:32px;font-size: 16px;vertical-align: middle">查询: </label>
        <input id="search" type="input" placeholder='Input id then "Enter"'/>
        <Button class="inp" id="random" type="primary" value="随机分布">随机分布</Button>
        <Button class="inp" id="cross" type="primary" value="减少交叉">减少交叉</Button>
        <Button class="inp" id='symmetric' type="primary" value="增加对称性">增加对称性</Button>
      </div>
      <h3>政企支付关系图</h3>
    </div>
    <div id="box">
...
<script>
  export default {
    ...
    mounted() {
      ...
      this.$axios.get("/api/eg/fen/dataTwo.json", {}).then((data) => {
        this.$set(this, 'loading', false);
        this.$Message.success('数据请求成功!');
        this.$set(this, "dataO", JSON.parse(JSON.stringify(data.data.data)));
        this.drawingPic();
      }).catch(() => {
        this.$set(this, 'loading', false);
    ...
    methods: {
      drawingPic() {
        ...
        searchInput.onkeydown = function (ev) {
          let e = ev || event;
          if (e.keyCode === 13) {
            const idNum = parseInt(searchInput.value);
            let flag = -1;
            for (let i = 0; i < nodes.length; i++) {
              if (nodes[i].id === idNum) {
                flag = i;
        ...
        function methodCross(nodesSection) {
          ...
          for (let i = 0; i < nodes.length; i++) {
            // nodes[i].degree = Math.sum(matrix[i]);
            let sum = 0;
            matrixV[i].forEach(function (item) {
              sum += item;
            });
            nodes[i].degree = sum;
          }
        ...
        function createElementDiv(dataInfo, m, nodes, links, centerNodes) {
          ...
          if (dataInfo.name) {
            ...
            h4.innerHTML = dataInfo.name + '<a href="https://kankan.fun/" style="color:blue">查看详细信息>></a>';
          ...
          let x = nodes[m].positionX > w/2 ? (nodes[m].positionX -250) : nodes[m].positionX + 50;
          let y = nodes[m].positionY > h*2/3 ? (nodes[m].positionY - 160) : nodes[m].positionY;
