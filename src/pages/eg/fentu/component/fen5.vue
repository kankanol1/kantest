      ...
      drawing() {
        ...
        function init() {
          ...
          for (let i = 0; i < centerNodes.length; i++) {
            ...
            let alpha = Math.atan((ratioH - nodes[flag].positionY) / (ratioW - nodes[flag].positionX));
            let n = 0;//标记
            let outL = 1;//放大系数
            for (let j = 0; j < nodes.length; j++) {
              if (matrix[flag][j] === 1 && nodes[j].degree === 1) {
                ...
                if (nodes[flag].positionX - ratioW > 0 && nodes[flag].positionY - ratioH <= 0) {
                  if (n % 2) {
                    nodes[j].positionX = ratioW + scaleC * Math.cos(alpha - chang);
                    nodes[j].positionY = ratioH + scaleC * Math.sin(alpha - chang - Math.PI / 2);
                  } else {
                    nodes[j].positionX = ratioW + scaleC * Math.cos(alpha + chang);
                    nodes[j].positionY = ratioH + scaleC * Math.sin(alpha + chang - Math.PI / 2);
                  }
                ...
                if (nodes[flag].positionX - ratioW <= 0 && nodes[flag].positionY - ratioH <= 0) {
                  if (n % 2) {
                    nodes[j].positionX = ratioW + scaleC * Math.cos(alpha - chang + Math.PI);
              ...
              if (array.length <= 1) {
                ...
                for (let j = iIndex + 1; j < array.length; j++) {
                  ...
                  if ((arr.size + array[j].size) > setA.size) {
                    ...
                    arrN.push(j);
                    array[j].forEach((value) => arr.add(value));
                  }
          ...
          function createElementDiv(dataInfo, m, nodes, links, centerNodes) {
            ...
            if (dataInfo.name) {
              ...
              h4.innerHTML = dataInfo.name + '<a href="http://kankan.fun/">查看详细信息>></a>';
            ...
            oDiv.style.left = nodes[m].positionX + 5 + 'px';
            oDiv.style.top = nodes[m].positionY + 5 + 'px';
            ...
            for (let i = 0; i < nodes.length; i++) {
              for (let j = 0; j < links.length; j++) {
                ...
                if (nodes[i].id.toString() == idF) {
                  ...
                  if (flagLine > -1) {
                    drawLine(nodes[i], nodes[flagLine], linkPay, '#000', 0.5);
                  }
                }
              }
            }
