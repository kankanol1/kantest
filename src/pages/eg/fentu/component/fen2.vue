    ...
    methods: {
      drawingPic() {
        ...
        function rectangle() {
          ...
          let juY = l + l;
          for (let i = 0; i < nodes.length; i++) {
            nodes[i].positionX = juX;
            nodes[i].positionY = juY;
        ...
        function createElementDiv(dataInfo, m, nodes, links, centerNodes) {
          ...
          oDiv.style.width = 250 + 'px';
          let x = nodes[m].positionX > w/2 ? (nodes[m].positionX - 260) : nodes[m].positionX+10;
          let y = nodes[m].positionY > h/2 ? (nodes[m].positionY - 160) : nodes[m].positionY;
